    ...
    <div class='search-component' v-show="show.show">
        <div class='search-header'>
            ...
            <div class='search-wrap'>
                ...
                <button type="button" class='search-btn' @click="searchHandle"></button>
            </div>
        </div>
        <div class='search-main'>
            <div class='search-title-wrap'>
                <div class='search-title'>最近搜索</div>
                <div class='bin' @click="clearHistorySearch"></div>
            </div>
        ...
        <div class='search-main'>
            ...
            <div class='search-keywords-wrap'>
                <div class='keywords' v-for="(item,index) in hot_keywords.slice(1)" :key="index"
                     @click="searchHandle($event,item.title)">{{item.title}}
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        computed: {
            ...mapState({
                ...
                hot_keywords: state => state.search.hot_keywords,
            })
        },
        methods: {
            ...mapActions({
                getHotKeywords: 'search/getHotSearchData'
            }),
            ...mapMutations({
                setHistoryKeywords: 'search/SET_HISTORY_KEYWORDS',
                clearHistoryKeywords: 'search/CLEAR_HISTORY_KEYWORDS',
            }),
            // 搜索事件
            searchHandle(e,hotKeyword) {
                let isValid = this.validKeyword(hotKeyword || this.keyword)
                if (isValid) {
                    this.setHistoryKeywords({keyword: hotKeyword|| this.keyword})
                }
                console.log(hotKeyword)
                if(hotKeyword){
                    this.show.show = false
                    this.goSearch(hotKeyword)
                }
            ...
            validKeyword(keyword) {
                console.log(keyword)
                let isValid = keyword.trim().length > 0
                let isNotInclude = this.historyKeywords.findIndex(item => item === keyword) < 0
                return isValid && isNotInclude
            },
            // 清除搜索历史记录
            clearHistorySearch() {
            ...
            goSearch(item) {
                this.show.show = false
                if(this.$route.name==='goods-search'){
                    this.$router.replace('/goods/search?keyword='+item)
                }else {
                    this.$router.push('/goods/search?keyword='+item)
                }
