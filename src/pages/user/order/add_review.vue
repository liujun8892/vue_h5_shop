...
<script>
    ...
    export default {
        ...
        computed:{
            ...mapState({
                reviewServices:state=>state.order.reviewOptions,
                uid:state=>state.user.uid
            })
        },
        methods:{
            ...mapActions({
                getReviewService:"order/_getCommentOptions",
                addReview:"order/_submitComment"
            }),
            ...mapMutations({
            ...
            submit(){
                ...
                if(this.isSubmit){
                    ...
                    this.addReview({gid:this.gid,content:this.content,ordernum:this.ordernum,rsdata:JSON.stringify(rsdata),success:(res)=>{
                            if(res.code===200){
                                this.$toast({ message:res.data,duration:1000,onClose:()=>{this.$router.go(-1)}})
                            }else{
                                Toast(res.data);
                            }
                        }});
