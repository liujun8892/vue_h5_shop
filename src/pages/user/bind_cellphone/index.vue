<template>
    <div class="page">
        <SubHeader title="绑定/修改手机"></SubHeader>
        <div class='main'>
            <div class='tip'>
                <div class='icon'></div>
                <div class='text'>新手机号验证后，输入6位不为空的验证码，即可绑定成功！</div>
            </div>
            <div class='input-wrap' style="margin-top:0.5rem">
                <input type="tel" class='cellphone' @input="checkCellphone" placeholder='绑定/修改手机号' v-model="cellphone" />
            </div>
            <div class='input-wrap' style="margin-top:0.2rem">
                <input type="text" class='code' placeholder='请输入短信验证码' v-model="msgCode" @input="handleVCode" maxlength="6" />
...
<script>
    ...
    export default {
        ...
        methods:{
            ...mapActions({
                isReg:"user/uValidPhone",
                updateCellphone:"user/_uploadPhoneNumber"
            }),
            //获取短信验证码
            async getMsgCode(){
            ...
            async submit(){
                ...
                let regData=await this.isReg({phoneNumber:this.cellphone});
                if(regData.data.isreg==='1'){
                    this.$toast("此手机号已注册过，请更换手机号");
                    return;
                }
